...
- **关联**：chinese-calendar 数据同样覆盖至 2026-12-31（2027 起节假日 chip 优雅降级，实测不崩）——两者同属"年末数据维护"族
- **维护动作（每年年末执行）**：①升级 chinese-calendar 依赖获取新年份数据 ②更新 base.json 春节条目为已确认的次年正月初一公历日期
- **根治方向（择机）**：春节目标改由 lunar 库运行时派生（LunarDate 转正月初一公历日期），消除静态维护
- **状态**：📌 登记（FIX004.8；不阻塞功能，2027-11 前处理即可）
