...
单包 Python 桌面应用：基于加速倍率的时间膨胀时钟（PyQt6，中文界面为主）。无 lint/格式脚本、无构建步骤；单元测试用 pytest（tests/，54 用例）。2026-09-10 起接入 DeepTransHub 工作流体系（文档四件套 + Commit 提交规范 + `.agents/skills/` 项目级技能）。

## 运行与验证

- 入口 `main.py`：GUI 为默认模式，CLI 用 `--cli`；版本号单一来源在 `config/static/base.json`（`base["version"]`，当前 `0.4.7.2`），各模块（main.py --version/窗口标题/托盘 toolTip）一律从配置读取，代码中不得出现版本字符串
...
- 单元测试：`.\.venv\Scripts\python.exe -m pytest tests/ -v`（54 用例覆盖 time_dilation/chinese_calendar/settings/alarm_service/weather_service/file_utils；依赖 `tests/requirements-dev.txt` 的 pytest）
