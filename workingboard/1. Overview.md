...
accelworld_gui.py         904 行     PyQt6图形界面，主窗口和组件
accelworld_config.py      136 行     配置文件管理，JSON格式持久化
accelworld_weather.py     199 行     天气服务，Open-Meteo API集成
─────────────────────────────────────────────────────────
合计                      1675 行    Python代码总计
```
...
├── accelworld_calc.py            核心计算（169行）
├── accelworld_date.py            日期农历（154行）
├── accelworld_gui.py             图形界面（904行）
├── accelworld_config.py          配置管理（136行）
