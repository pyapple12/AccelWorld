...
## PL008：GLSL 液态玻璃渲染（愿景已确认，探针待跑，2026-09-12 记录）
...
> 状态：🟨 愿景已确认（2026-09-12 用户拍板"整窗液态玻璃 + 侧栏药丸"），探针验证后正式立项；任务清单入 x.progress.md 对应任务组

### 立项讨论补充（2026-09-12，用户愿景确认）

> 用户拍板：目标即"整窗液态玻璃"——窗口本体是一块大玻璃面板，左侧栏为玻璃药丸按钮组，内容卡片为嵌套玻璃面板（参考 Liquid Glass UI Kit 形态图）；现状整窗仅磨砂 Acrylic + 实底背景，达不到该观感

**范围界定（换引擎，不换体系）**：GLSL 只接管"玻璃光学"渲染层（QPainter 光学近似 → QOpenGLWidget + 片段着色器）；qfw 控件体系、interface 七域分层、六页业务逻辑、ui.json token、鎏金/天青视觉语言全部不动；栅格路径保留为无 GL 环境降级（能力探测切换）

**目标形态与落地架构**：

- 整窗一块 QOpenGLWidget GL 画布，负责画所有"玻璃"：背景光场（升级为可折射的丰富内容：渐变+云雾+光斑，可扩展用户选图）→ 高斯模糊 → SDF 圆角玻璃面板（侧栏药丸 + 内容卡同管线批量画）→ 折射位移（lensing）+ 色散 + rim 高光
- 侧栏导航从 qfw NavigationInterface 换成**自制玻璃药丸按钮组**（唯一替换的 qfw 组件；垂直图标+文字列，交互接 interface 现有 API）
- Qt 控件透明底叠 GL 画布上方，负责文字渲染与交互命中（文字仍由 Qt 画，保证清晰度）
- DWM Acrylic 保留打底，桌面壁纸的模糊从最底层透上

**参考图实现度评估**（用户提供 4 张参考图）：

| 形态              | 要点                             | 实现度                                                                            |
| ----------------- | -------------------------------- | --------------------------------------------------------------------------------- |
| Pink Sky 玻璃卡   | 边缘折射弯折背景 + 色散 + 高光   | ≈80%（前提背景在窗内：光场层做丰富；折射对象是窗内光场，非壁纸）                  |
| 厚玻璃按钮套件    | 强色散 + 厚玻璃体积感 + 真实投影 | ≈60%（色散/内高光/边缘光可做；3D 体积感实时渲染打折，取神似；原图疑似离线渲染图） |
| Liquid Glass 套件 | 软渐变玻璃 + 轻折射 + 多层嵌套   | ≈70%（GLSL 标准操作，即本项目目标形态）                                           |
| 天气卡贴壁纸      | 模糊壁纸透过 + 轻高光            | ≈90%（整窗 Acrylic + 薄卡 + GLSL 高光，最接近现有路线）                           |

**系统边界（重要）**：桌面壁纸无法逐卡片弯折——DWM Acrylic 只能整窗模糊；逐卡折射的对象是**窗内光场层**。要"折射壁纸"的观感，靠整窗 Acrylic 透壁纸 + 卡片 GLSL 只加高光/轻折射

**探针验证项（立项前置，约 1 天，产出用户真机验收）**：

1. 透明 GL 画布与 DWM Acrylic 共存（第一块多米诺；不行则降级"GL 画布自带背景"）
2. Qt 控件透明底叠 GL 表面的文字清晰度
3. 单面板折射 + 色散 + 高光样例，对照参考图验收

**实施批次（探针过关后细化入 x.progress.md）**：GL 画布骨架 → 玻璃材质管线 → 侧栏玻璃药丸 → 六页迁移 → 降级路径与全量回归

**附加风险**：GL 上下文引入后 y.problems#6 复测；PyInstaller + OpenGL 打包验证（关联既有顺延项）
