...
## PL008 预记：GLSL 液态玻璃渲染（待讨论，2026-09-12 记录）

> 背景：「时之砂」栅格路线已到能力上限——真折射无法实现、offscreen 渐变栅格存在随机崩溃缺陷、色带依赖噪点压制；Qt 社区成熟方案指向 QOpenGLWidget + GLSL 多通道渲染（参考 CSDN 149879733 / 152576978、知乎 1977834539406606475、kube.io 折射拆解）
> 方向草案：光场位图上传 GL 纹理 → 片段着色器实现高斯模糊 / UV 折射位移（lensing）/ SDF 圆角 rim / specular 高光 / RGB 色散；着色器浮点精度 + dither 天然无色带；offscreen 无 GL 环境自动降级栅格路径（能力探测切换）
> 前置验证：真机 GL 探针先行（单卡折射样例，约 1 天）——确认驱动兼容与观感达标后，再正式立项细化
> 风险：OpenGL 驱动依赖；PyInstaller + OpenGL 打包验证（关联既有顺延项）；y.problems#6 复测
> 状态：⬜ 待讨论，未立项；正式立项后任务清单入 x.progress.md 对应任务组
