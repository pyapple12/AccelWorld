...
> 当前版本：0.5.6.1（UI3.0 铺开：玻璃药丸导航 + 六页 GL 迁移，gl_enabled 默认开启）
...
- [ ] PL010.01 性能预算收口 —— paintGL 内 QElapsedTimer 计时（GL/栅格两模式各录 100 帧均值入档 .temp/perf\_\*.txt）；静态帧缓存断言（glass_scene 无 dirty 时 paintGL 直接 return）；验证：重绘 <8ms + 静态零重绘探针
